...
import org.mybatis.generator.support.api.GenServiceContext;
...
public class Context extends PropertyHolder {
    ...
    public List<GenServiceContext> generateServiceFile() {
        JavaServiceGeneratorConfiguration serviceCfg = getJavaServiceGeneratorConfiguration();
        JavaModelGeneratorConfiguration modelCfg = getJavaModelGeneratorConfiguration();
        JavaClientGeneratorConfiguration mapperCfg = getJavaClientGeneratorConfiguration();
        List<GenServiceContext> genServiceContexts = new ArrayList<>();
        List<IntrospectedTable> introspectedTables = this.introspectedTables;
        String implementationPackage = serviceCfg.getProperties().getProperty("implementationPackage");
        if (!introspectedTables.isEmpty()) {
            for (IntrospectedTable itable : introspectedTables) {
                String domainObjectName = itable.getFullyQualifiedTable().getDomainObjectName();
                String mapperName = domainObjectName + "Mapper";
                String exampleName = domainObjectName + "Example";
                PackageContext p = new PackageContext(serviceCfg.getTargetPackage(), mapperCfg.getTargetPackage()+"."+mapperName, modelCfg.getTargetPackage()+"."+domainObjectName,
                        modelCfg.getTargetPackage()+"."+exampleName, serviceCfg.getTargetProject(),implementationPackage);
                TableContext tableContext = new TableContext(domainObjectName + "Service", domainObjectName + "Mapper", domainObjectName, domainObjectName + "Example", null == itable.getRemarks() ? "" :itable.getRemarks());
                GenServiceContext genServiceContext = new GenServiceContext(p, tableContext);
                genServiceContexts.add(genServiceContext);
            }
        }
        return genServiceContexts;
    }
}
